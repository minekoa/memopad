...
# 環境に併せて設定すること！
datadir = r'C:\Documents and Settings\zoni\My Documents\Dropbox\tools\memopad\memo'
workdir = './'
...
class MemoPad(Subject):
    ...
    def saveImage(self):
        ...
        for i in self.memos:
            f = open( '%s' % self.memoToFilename(i), 'w' )
    ...
    def sendToTrashboxImage(self):
        ...
        for i in self.trashbox:
            try:
                os.remove( '%s' % self.memoToFilename(i) )

                f = open( 'trash/%s' % self.memoToFilename(i), 'w' )
                f.write( i.getText().encode('ms932') )
...
class MemoPadFrame( Frame ):
    ...
    def __init__(self, master=None):
        ...
        self.master.title( 'MemoPad %d.%d.%ds' % self.version )
